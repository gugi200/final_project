...
class SensorMgr():
    ...
    def update(self):
        ...
        prediction, prob, _ = self.makePrediction(data)  
        if prob<0.75:
    ...
    def createAnimationFrame(self): 
        frame = Frame(self.root, background='#ffffff', highlightbackground='#000000', 
            highlightthickness=2, width=400, height=600, padx=3, pady=3)
    ...
    def importModel(self):
        ...
        if self.online:
            ...
            if 'googlenet' in self.model_path:
            ...
            if 'mobilenet_v3_large' in self.model_path:
                weight = list(torchvision.models.get_model_weights('mobilenet_v3_large'))[-1]
                self.lodaed_model = torch.hub.load('pytorch/vision', 'mobilenet_v3_large', weight)
            ...
            self.lodaed_model = torch.load(self.model_path)
